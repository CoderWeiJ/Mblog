  ...
  methods: {
    async getData() {
      try {
        ...
        if (res.success) {
          this.pv = res.result.pv
          this.uv = res.result.uv
          this.blogCount = res.result.blogCount
          this.commentCount = res.result.commentCount
          // 渲染分类数据
          this.categoryOption.legend.data = res.result.category.legend
          this.categoryOption.series[0].data = res.result.category.series
          this.initCategoryEcharts()
          // 渲染标签数据
          this.tagOption.legend.data = res.result.tag.legend
          this.tagOption.series[0]
          this.initTagEcharts()
          // 渲染访客地图数据
          const mapData = this.convertData(res.result.cityVisitor)
          this.mapOption.series[1].data = mapData
          this.mapOption.series[2].data = mapData.splice(0, 5)
          this.initMapEcharts()
          // 渲染一周访问量数据
          this.visitRecordOption.xAxis.data = res.result.visitRecord.date
          this.visitRecordOption.series[0].data = res.result.visitRecord.pv
          this.visitRecordOption.series[1].data = res.result.visitRecord.uv
          this.initVisitRecordEcharts()
        }
      } catch (error) {
        console.error('Dashboard [getData] Error: ', error.message)
      }
    },
    initCategoryEcharts() {
