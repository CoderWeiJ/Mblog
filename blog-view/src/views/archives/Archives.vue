<template>
  ...
  <div>
    ...
    <div class="ui attached segment">
      <div class="timeline">
        <div :class="colorObj[index % 5]" v-for="(item, index) in blogMap" :key="index">
          <div class="tl-header">
            <a href="javascript:;" class="ui large label m-text-500">{{ item.date }}</a>
          </div>
          <div class="tl-item" v-for="blog in item.blogs" :key="blog.id">
            <div class="tl-wrap">
              <span class="tl-date">{{ blog.createdAt }}</span>
              <a @click.prevent="toBlog(blog)">
...
const blogMap = ref({})
const count = ref(0)
const colorObj = reactive({ 0: 'tl-blue', 1: 'tl-dark', 2: 'tl-green', 3: 'tl-purple', 4: 'tl-red' })

// 逻辑处理
const useArchivesEffect: Function = () => {
  const getArchivesData: Function = async () => {
    try {
      let res = await getArchives()
      if (res.success) {
        blogMap.value = res.result.blogMap
  ...
  const toBlog: Function = (blog: Object) => store.dispatch('goBlogPage', blog)
  return { getArchivesData, toBlog }
}
const { getArchivesData, toBlog } = useArchivesEffect()
// 生命周期
onBeforeMount(() => getArchivesData())
</script>
